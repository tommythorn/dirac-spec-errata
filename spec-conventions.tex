...
contained in Appendix \ref{parsediagrams},
 that complement the normative stream syntax definitions. The parsing process 
...
the elements of a map $m$ can be accessed by $\args(m)$, so that, for example, $args(p)$ returns$\{Y,U,V\}$.
...
\subsubsection{Map and array functions and operators}
