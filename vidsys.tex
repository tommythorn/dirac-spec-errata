...
Y & = & 2*(t+(Cg\gg1)) \approx I_G-(I_R+I_B)/2+(I_R+I_B)=I_R/2+I_G+I_B/2
\end{eqnarray*}

Note that these matrix operations involve a doubling of the luma range in
order to give it the same range as chroma, which gains a bit through
the subtraction. So for 8-bit RGB ($I_R$, $I_G$, $I_B$) values, $Y$, $Co$ and
$Cg$ will be 9 bits. (Note that this may differ from other implementations
which contain different resolutions for luma and chroma data).

Note also that $\LumaExcursion$ and $\ChromaExcursion$ are not required for
these scaling operations. They should be set to $2^\VideoDepth$.
