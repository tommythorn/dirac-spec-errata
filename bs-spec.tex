...
Parse Info header data. This header is byte-aligned. It occurs at the beginning
of a sequence, at the end of a sequence, before an Access Unit header, 
and before each set of picture data. It is used to navigate through the stream (Section \ref{picturedec}).
...
\bsCODE{\NextParseOffset=read\_uint\_lit(3)}
\bsCODE{\PrevParseOffset=read\_uint\_lit(3)}
\end{pseudo}
...
\bsRET{((\ParseCode \&\text{0x18})==\text{0x10})}
\end{pseudo}
...
This section specifies the structure of the Access Unit header. This Access Unit header is byte aligned.
Parsing this header consists in
reading the Access Unit parameters (parse, source and sequence parameters) and initialising 
...
to decode a sequence. Applicable values of profile and level are specified in Appendix
\ref{profilelevel}.
...
For example, an image width of 360 may be encoded as per Section {imagedimensions},
...
The supported chroma formats are specified in Table \ref{chromaformats}:
...
\hline
\end{tabular}
\caption{Supported chroma formats}\label{chromaformats}
\end{table}
...
The Access Unit source parameters consist of: the scan format, frame rate,
aspect ratio, clean area, signal range and colour specification. These parameters
...
The picture header is byte aligned and follows a Parse Info header with a picture parse code.
The process for parsing the picture header is as follows:
...
signalled within the stream. Both elements are byte-aligned within the stream.
...
powers of 2 other than 1
\item $\ChromaXBlen-\ChromaXBsep$ and $\ChromaYBlen-\ChromaYBsep$ shall be
powers of 2 other than 1
\end{enumerate}

\begin{informative}
Note that these requirements do not preclude length from equalling separation, i.e.
motion compensation blocks are not overlapped. This may improve quality at higher
bitrates.
\begin{comment}
...
[Note that all the stuff about recomputing the chroma block lengths is now redundant since
we allow non-overlapping blocks as well]
...
Their interpretion and the process for generating a global motion vector field is specified in Section \ref{globalmv}. 
...
(Section \ref{pixelpredict}). 


%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Wavelet transform}
\label{wavelettransform}
The $wavelet\_tranform()$ function parses metadata determining the wavelet
...
\bsIF{\ZeroResidual = \false}
    ...
    \bsCODE{\YTransform = transform\_data()}{\ref{transformdata}}

    \bsCODE{\ComponentWidth = \ChromaWidth}
    \bsCODE{\ComponentHeight = \ChromaHeight}
    \bsCODE{\UTransform = transform\_data()}{\ref{transformdata}}
    \bsCODE{\VTransform = transform\_data()}{\ref{transformdata}}
\end{pseudo}
...
and how subbands are spatially partitioned:
...
\begin{comment}
[To do: Tim suggests having 0 as a terminator meaning: values are the same from here on]
\end{comment}
