...
Scan format parameters are concerned with interlace. 

If $\SSourceSampling=0$, then the video should be displayed as progressive video. 

If $\SSourceSampling=1$, then the video should be displayed as interlaced video. 

Values of $\SSourceSampling$ greater than 1 shall be reserved.
...
\bsIF{custom\_scan\_format\_flag==\true}
    ...
    \bsIF{\SSourceSampling==1}
         \bsITEM{\STopFieldFirst}{bool}{}
    \bsEND
\bsEND
\end{pseudo}

If the custom scan format flag is set to $\true$, the scan format parameters defined by 
