...
on prior pictures in the stream.

A sequence header shall contain metadata describing the coded sequence 
and metadata needed to decode the stream. It shall be the first data unit in a sequence.
...
The first picture after each sequence header shall be an intra picture.
 
The sequence decoding and parsing processes start by extracting and decoding the 
information contained in a sequence header. Picture data units contain the additional 
information needed to decode a single output picture.
...
\item The second MS bit (bit 6) is used to indicate whether arithmetic coding is used and only applies to pictures. Core syntax may optionally use arithmetic coding. Low delay syntax does not use arithmetic coding. The permutation of the two MSBs which might indicate low delay syntax with arithmetic coding is reserved.
...
interlaced coding) and initializing the decoder parameters. The decoder 
parameters are initialized in the $coding_parameters()$ process (below).
Sequence parameters shall remain constant throughout a sequence, 
so the sequence header need only be read once per sequence, and 
other sequence headers in the same sequence may be ignored .
...
\bsITEM{interlaced\_coding}{bool}{\ref{interlacedcoding}}
\bsCODE{coding\_parameters(\VideoParams,interlaced\_coding)}{\ref{interlacecoding}}
\bsRET{\VideoParams}
\end{pseudo}
...
flexibility, source parameters may override some or all of the parameters indicated 
by the base video format.
...
Interlaced coding indicates whether the video has been coded as a sequence of frames or fields.

Once the base video format, source parameters and interlaced coding have 
...
Appendix \ref{videoformatdefaults}, indicating default video parameters corresponding to the video source. These predefined parameters may be overridden by subsequent metadata present in the stream as defined in subsequent sections below.
...
The source parameters shall comprise frame size, sampling format, scan format, frame rate,
 aspect ratio, clean area, signal range and color specification. The frame size, sampling
...
\bsCODE{sampling\_format(\VideoParams)}{\ref{chromaformat}}
\bsCODE{scan\_format(\VideoParams)}{\ref{scanformat}}
\bsCODE{frame\_rate(\VideoParams)}{\ref{framerate}}
\bsCODE{aspect\_ratio(\VideoParams)}{\ref{aspectratio}}
...
\begin{pseudo}{sampling\_format}{\VideoParams}
\bsITEM{custom\_sampling\_format\_flag}{bool}{}
\bsIF{custom\_sampling\_format\_flag==\true}
     \bsITEM{\SSamplingFormatIndex}{uint}{}
\bsEND
\end{pseudo}
...
\begin{tabular}{|c|c|}
\hline
\rowcolor[gray]{0.75}sampling\_format\_index & {\bf Chroma format} \\
\hline
0 & 4:4:4 \\
\hline
1 & 4:2:2 \\
...
video should be displayed as interlaced video. The process for parsing the scan format parameters shall be as follows:
...
the base video format values shall be overridden by the new values.
...
The process for decoding the aspect ratio parameters shall be defined as follows:

\begin{pseudo}{aspect\_ratio}{\VideoParams}
\bsITEM{custom\_aspect\_ratio\_flag}{bool}{}
\bsIF{custom\_apect\_ratio\_flag==\true}
    \bsITEM{index}{uint}{}
    \bsIF{index == 0}
        \bsITEM{\SAspectRatioNumer}{uint}{}
        \bsITEM{\SAspectRatioDenom}{uint}{}
    \bsELSE
        \bsCODE{preset\_aspect\_ratio(\VideoParams,index)}
    \bsEND
\bsEND
\end{pseudo}
...
For $index>0$, $preset\_aspect\_ratio(\VideoParams, index)$ shall set the pixel aspect ratio value elements of $\VideoParams$ as specified in Table \ref{table:aspectratiovalues}.
...
\begin{informative}
The aspect ratio is defined as the ratio of the aspect ratio parameters:
		\[\SAspectRatioNumer : \SAspectRatioDenom\]
The aspect ratio value defines the intended pixel aspect ratio of the pixels such that the viewed picture has no geometric distortion. The pixel aspect ratio of an image is the ratio of the spacing of horizontal samples (pixels) to the spacing of vertical samples (picture lines) on the display device. Pixel aspect ratios (PARs) are fundamental properties of sampled images because they determine the displayed shape of objects in the image. Failure to use the right PAR will result in distorted images, for example circles will be displayed as ellipses etc. 
...
The interlaced coding part of the sequence header shall determine whether source video is coded as frames or fields. 

If the interlaced coding flag is $\true$ then pictures shall correspond to fields; otherwise 
pictures shall correspond to frames. 
...
\begin{pseudo}{coding\_parameters}{\VideoParams, interlaced\_coding}
\bsCODE{picture\_dimensions(\VideoParams, interlaced\_coding)}{\ref{picturedimensions}}
...
Global motion parameters $\GlobalParams$ shall consist of three elements: 
...
The $zoom\_rotate\_shear()$ process extracts a linear matrix element and shall be
