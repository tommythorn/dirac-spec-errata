...
\bsCODE{\ACode =\text{ 0x0}}{No- TBC-read 16 bits}
...
In all of these cases the interval can be doubled in size to increase accuracy. In the first case, it is doubled from zero ($x\arrow 2*x$); 
in the second it is doubled from 1 ($x\arrow 2*x-1$); and in the third it is doubled from 1/2 ($x\arrow 2x-0.5$).[TBC]
