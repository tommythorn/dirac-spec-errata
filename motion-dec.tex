...
\bsFOR{q=0}{block\_count-1}
   \bsFOR{p=0}{block\_count-1}
        \bsCODE{block\_decode(y+q*step, x+p*step)}{\ref{blockdecoding}}
...
The $block\_vectors(ypos, xpos)$ process parses the block motion vectors for the block at position $(xpos, ypos)$.

\begin{pseudo}{block\_vectors}{ypos, xpos}
