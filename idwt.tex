...
2. Type 3, $s=4$, $t_{-1}=-1, t_0=9, t_1=9, t_2=-1$ i.e. \\
...
\end{tabular}
\caption{$\WaveletIndex==0$: Deslauriers-Debuc (9,5) lifting stages and shift values}\label{filtertable0}
\end{table}
...
Lifting steps: \\
\begin{tabular}{l}
1. Type 2, $s=5, t_{-1}=-1, t_0=9, t_1=9, t_2=-1$ i.e. \\
...
Lifting steps:\\
\begin{tabular}{l}
1. Type 3, $s=8, t_{-3}=-2, t_{-2}=10, t_{-1}=-25, t_0=81, t_1=81, t_2=-25, t_3=10, t_4=-2$ i.e. \\
$ \begin{array}{rcl} A[2*n+1] & += & (-2*(A[2*n-6]+A[2*n+8])+10*(A[2*n-4]+A[2*n+6])\\
                                             & &       -25*(A[2*n-2]+A[2*n+4])+81*(A[2*n]+A[2*n+2])+128)\gg 8 \end{array}$ \\
1. Type 2,  $s=8, t_{-3}=-8, t_{-2}=21, t_{-1}=-46, t_0=161, t_1=161, t_2=-46, t_3=21, t_4=-8$ i.e. \\
...
the final scaling stages), it is a very close approximation. Unlike the JPEG2000 implementation, 
these filters can therefore be used for lossless as well as lossy compression, and lossy
compression performance is near-indentical to the real-valued filter. The integer lifting implementation
