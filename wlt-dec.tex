...
\bsCODE{data=initialise\_wavelet\_data()}{\ref{wltinit}}
\bsCODE{subband\_decode(data, 0, \LL)}{\ref{subbanddecodeprocess}}    
\bsFOR{level=1}{\TransformDepth}
  \bsFOREACH{orient}{\LH,\HL,\HH}
    \bsCODE{subband\_decode(data, level, orient)}{\ref{subbanddecodeprocess}}
  \bsEND
\bsEND
\bsCODE{byte\_align()}{[Correct?]}
\bsRET{data}
\end{pseudo}
...
\bsIF{length>0}
    ...
    \bsCODE{vol=subband\_height(level)*subband\_width(level)}
...
\bsIF{\CoefficientCount == \CoefficientReset}
  ...
  \bsFOR{i=0}{len(\AContexts)-1}
    \bsCODE{rscale\_context(i)}{\ref{arithcontexts}}
  \bsEND
\bsEND
\end{pseudo}
