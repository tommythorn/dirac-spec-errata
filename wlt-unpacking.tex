...
unpacking (parsing, entropy decoding and inverse
quantising) of wavelet coefficient data
from the Dirac stream. . Wavelet coefficients shall be signed integer 
values and shall be extracted using the integer VLC and (optionally) arithmetic decoding functions  defined in Appendix \ref{dataenc}. 
...
\begin{itemize}
\item In the core syntax, coefficients shall be grouped within individual
subbands, representing a range of spatial frequencies, from the
lowest to the highest. A full set of subbands shall be encoded for each
video component in turn. 

\item In the low delay syntax, coefficients
shall be grouped into `slices' which represent coefficients pertaining
to an area of the picture. Each slice shall contain data for all video 
components and spatial frequency bands. Unpacking a slice 
...
Unpacked wavelet coefficient data shall be stored in the state variables $\YTransform$, $COneTransform$ and $CTwoTransform$ for subsequent picture decoding 
...
All other levels shall consist of 3 subbands of orientation \LH, \HL, 
...
\begin{pseudo}{subband\_coeffs}{coeff\_data,level,orient,length}
...
$codeblock(band,parent,level, orient,x,y,quant\_index)$

This process unpacks coefficients within the codeblock at position $(x,y)$.
The codeblock dimensions shall be as defined in Section \ref{codeblockdimensions}.

The process for unpacking coefficients within the codeblock shall be defined as follows: 

\begin{pseudo}{codeblock}{coeff\_data,level,orient,x,y,quant\_index}
\bsCODE{skipped=zero\_flag(level)}{\ref{zeroblockflag}}
\bsIF{skipped==\false}
  \bsCODE{quant\_idx += codeblock\_quant\_offset()}{\ref{blockquantidx}}
  \bsFOR{v=cb\_top(y,band,level)}{cb\_bottom(y,band,level)-1}{\ref{codeblockdimensions}}
    \bsFOR{h=cb\_left(x,band,level)}{cb\_right(x,band,level)-1}{\ref{codeblockdimensions}}
          \bsCODE{coeff\_unpack(coeff\_data,level,orient,quant_idx,v,h)}{\ref{wltcoeff}}
      \bsEND
    \bsEND
  \bsEND
\bsEND
...
$quant\_context\_probs()$ shall return the context probability label set:
...
and neighbouring values are zero or non-zero. See Appendix \ref{arithdecoding}
...
\bsEND
\bsCODE{coeff\_data[level][orient][x][y] = inverse\_quant( quant\_coeff, quant\_index )}{\ref{invquant}}
\end{pseudo}
...
shall be as defined in Appendix \ref{arithcontexts}.

The map m shall comprise three elements, accessed by the labels: FOLLOW, DATA, and SIGN, where:
\begin{itemize}
\item $m$[FOLLOW] is an array of labels for accessing follow bit context probabilities
\item $m$[DATA] is a label for accessing data bit context probabilities
\item $m$[SIGN] is a label for accessing data bit context probabilities 
\end{itemize}

The elements of the map returned by 
\[select\_coeff\_contexts(zero\_nhood,parent,sign\_pred)\]

shall be as defined in Table \ref{contexttable} where the context labels are used to access
context probability values in the map $\AContexts$.
...
\hline
 $zero\_parent$ & $zero\_nhood$ & $sign\_pred$ & \multicolumn{2}{c|}{\bf{Context map}} \\
...
This section specifies the operation of the $slice(sx,sy)$ process for unpacking
...
This section specifies the $slice\_bytes(sx,sy)$ function for determining the number of bytes to be used for the slice at location $(sx,sy)$:
...
This section specifies how quantisers for individual subbands are determined from the quantisation matrix and the quantisation index.
...
This section specifies the operation of the $luma\_slice\_band( level,orient,sx,sy)$ for 
