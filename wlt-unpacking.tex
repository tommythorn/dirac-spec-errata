...
defined in Section \ref{parsecodes}. The result of this process shall be a set of fully
...
\bsIF{skipped==\false}
  ...
  \bsFOR{v=cb\_top(y,band,level)}{cb\_bottom(y,band,level)-1}{\ref{codeblockdimensions}}
    \bsFOR{h=cb\_left(x,band,level)}{cb\_right(x,band,level)-1}{\ref{codeblockdimensions}}
          \bsCODE{coeff\_unpack(coeff\_data,level,orient,quant\_idx,h,v)}{\ref{wltcoeff}}
      \bsEND
    \bsEND
  \bsEND
\bsEND
...
This section defines the $slice\_bytes(sx,sy)$ function for determining the number of bytes to be used for the slice at location $(sx,sy)$:
...
This section defines how quantisers for individual subbands are determined from the quantisation matrix and the quantisation index.
