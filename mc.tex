...
\bsCODE{sv[0] \gg= chroma\_h\_shift()}{\ref{chromaformat}}
\bsCODE{sv[1] \gg= chroma\_v\_shift()}{\ref{chromaformat}}
\bsRET{sv}
\end{pseudo}
...
In the case of YCgCo coding, bit depth may vary for luma and chroma components.
Hence the variable $\BitDepth$ is set equal to 
\[\LumaDepth\]
if the component $c$ is Y, and
\[\ChromaDepth\]
otherwise.
...
\bsFOR{y=0}{lenY-1}
    \bsFOR{x=0}{lenX-1}
        ...
        \bsCODE{pic[y][x] = \clip(pic[y][x], -2^{\BitDepth-1}, 2^{\BitDepth-1}-1)}
    \bsEND
\bsEND
\end{pseudo*}
...
calculations is \[\BitDepth+total\_wt\_bits+\RefsWeightPrecision\]
...
for interior blocks is as shown in Figure \ref{fig:weightprofile}.
...
\bsCODE{ref2[q][p] = \clip(ref2[q][p], -2^{\BitDepth-1}, 2^{\BitDepth-1}-1)}
\end{pseudo*}
...
\bsELSE
  ...
  \bsCODE{upref[q][p] = \clip(upref[q][p], -2^{\BitDepth-1}, 2^{\BitDepth-1}-1)}
\bsEND
\end{pseudo*}
