...
\bsCODE{sv.x = \rounddivide(v.x, chroma\_h\_factor() )}{\ref{chromaformat}}
\bsCODE{sv.x = \rounddivide(v.x, chroma\_v\_factor())}{\ref{chromaformat}}
\bsRET{sv}
\end{pseudo}
...
the block overlaps - selected. A Dirac decoder level (\ref{levels}) 
...
{\bf Case 1: $\BlockData[j][i][global]=\false$}. 
In this case, the block motion vectors are used to form a prediction.
Motion vectors for chroma components must be scaled 
according to the chroma scale factors. If $c=Y$, set
\begin{equation*}
mv= \BlockData[j][i][ref]
\end{equation*}
whereas if $c=U$ or $c=V$, set
\begin{equation*}
mv = chroma\_mv\_scale(\BlockData[j][i][ref])
\end{equation*}
as per Section \ref{chromamvscale}.

{\bf Case 2: $\BlockData[j][i][global]=\true$}. 
In this case, a motion vector is determined from the global
...
mv.y & = & y_1\gg (\alpha+\beta)
\end{eqnarray*}

If $c=Y$, then $mv$ is returned, otherwise 
$chroma\_mv\_scale(mv)$ (Section \ref{chromamvscale}) is returned.
