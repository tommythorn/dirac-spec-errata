[TO DO: Add in global motion compensation stuff]
...
\bsFOR{y=0}{lenY-1}
    \bsFOR{x=0}{lenX-1}
        \bsCODE{data[y][x] += pixel\_predict(y, x, data, ref1, ref2, c)}
        \bsCODE{data[y][x] = \clip(data[y][x], 0, 2^\VideoDepth-1)}
    \bsEND
\bsEND
\end{pseudo*}

For block indices $(i,j)$, define the set of elements in the corresponding
block by:
...
\begin{pseudo*}
\bsCODE{hbits  = log2(xblen-xbsep)+1}
\bsCODE{vbits  = log2(yblen-ybsep)+1}
\bsCODE{\alpha=hbits+vbits+\MVWeightPrecision}
\end{pseudo*}
...
\bsFORSUCH{(i,j)}{(x,y)\in B(i,j)}
    \bsCODE{p+=wt(i,j,x,y)*block\_pred(ref1, ref2, j, x, y, c)}{\ref{mcspatialweights},\ref{mcblock}}
    \bsCODE{p=(p+2^{\alpha-1})\gg \alpha}
\bsEND
\bsRET{p}
\end{pseudo}
...
\begin{pseudo*}
\bsCODE{mv1= \BlockData[j][i][ref1]}
\bsCODE{mv2= \BlockData[j][i][ref2]}
\end{pseudo*}

whereas if $c=U$ or $c=V$, set

\begin{pseudo*}
\bsCODE{mv1.x = (\BlockData[j][i][ref1].x +(chroma\_h_factor()//2) )//chroma\_h_factor()}{\ref{chromafactors}}
\bsCODE{mv1.y = (\BlockData[j][i][ref1].y +(chroma\_v_factor()//2) )//chroma\_v_factor()}{\ref{chromafactors}}
\bsCODE{mv2.x = (\BlockData[j][i][ref2].x +(chroma\_h_factor()//2) )//chroma\_h_factor()}{\ref{chromafactors}}
\bsCODE{mv2.y = (\BlockData[j][i][ref2].y +(chroma\_v_factor()//2) )//chroma\_v_factor()}{\ref{chromafactors}}
\end{pseudo*}

There are four cases, corresponding to
each mode:
...
%
%(w00*upref[cv][cu]+w01*upref[cv][cu1]+w10*upref[cv1][cu]+w11*upref[cv1][cu1]+2^{\MotionVectorPrecision-2})\gg(\MotionVectorPrecision-1)
%
%\begin{informative}
