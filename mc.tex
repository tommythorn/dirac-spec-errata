...
\bsFOR{y=\max(ystart,0)}{\min(ystop,\LenY)-1}
    \bsFOR{x=\max(xstart,0)}{\min(xstop,\LenX)-1}
        ...
        \bsIF{m==\Intra}
            ...
            \bsCODE{mc\_tmp[y][x]+=\BlockData[j][i][dc][c]*wt}
        \bsELSEIF{m==\RefOneOnly}
            \bsCODE{wt=(\RefOneWeight+\RefTwoWeight) * W[q][p]}
            \bsCODE{mc\_tmp[y][x]+=pixel\_pred(ref1, 1, i, j, x, y, c)*wt}{\ref{}}
        \bsELSEIF{m==\RefTwoOnly}
            \bsCODE{wt=(\RefOneWeight+\RefTwoWeight) * W[q][p]}
            \bsCODE{mc\_tmp[y][x]+=pixel\_pred(ref2, 2, i, j, x, y, c)*wt}{\ref{}}
        \bsELSE
...
This section specifies the function $spatial\_wt(i,j)$ for deriving the spatial weighting 
...
\begin{pseudo}{h\_wt}{i,x}
\bsCODE{max\_x=4*\XOffset}
\bsIF{i==0 \text{ and } x< \XBlen//2}
    \bsRET{max\_x}
\bsELSEIF{i==\BlocksX \text{ and } x\geq \XBlen//2}
    \bsRET{max\_x}
\bsELSE
    \bsRET{\clip(\XBlen-2\abs{x-\dfrac{(\XBlen-1)}{2}}, max\_x ) }
\bsEND
\end{pseudo}

The horizontal weighting function  shall be defined as follows:

\begin{pseudo}{v\_wt}{j,y}
\bsCODE{max\_y=4*\YOffset}
\bsIF{j==0 \text{ and } y< \YBlen//2}
    \bsRET{max\_y}
\bsELSEIF{j==\BlocksY \text{ and } y\geq \YBlen//2}
    \bsRET{max\_y}
\bsELSE
    \bsRET{\clip(\YBlen - 2\abs{y-\dfrac{(\YBlen-1)}{2}},0, max\_y) }
\bsEND
\end{pseudo}
...
[GOT HERE!!]
This section specifies the operation of the $pixel\_pred(ref, ref\_num, i, j, x, y, c)$ 
...
\bsELSE
  \bsCODE{px = (x+mv[0])\ll 1}
  \bsCODE{py = (y+mv[1])\ll 1}
\bsEND
\bsRET{upconvert(ref, px, py))}{\ref{upconvert}}
\end{pseudo}
...
\bsCODE{upref=interp2by2(ref)}{\ref{halfpel}}
...
\paragraph{Quarter- and eighth-pixel accurate motion vectors}
$\ $\newline
...
\paragraph{Half-pixel interpolation}
\label{halfpel}
$\ $\newline
...
\bsFOR{i=0}{4}
  \bsCODE{ypos=(q-1)//2-i}
  \bsCODE{ref2[q][p]+=t_i*ref[\clip(ypos,0,\height(ref)-1)][p]}
  \bsCODE{ypos=(q+1)//2+i}
  \bsCODE{ref2[q][p]+=t_i*ref[\clip(ypos,0,\height(ref)-1)][p]}
\bsEND
\bsCODE{ref2[q][p] += 128}
\bsCODE{ref2[q][p] \gg=8}
...
\bsELSE
  ...
  \bsFOR{i=0}{4}
    \bsCODE{xpos=(p-1)//2-i}
    \bsCODE{upref[q][p]+=t_i*ref2[q][\clip(xpos,0,\height(ref)-1)]}
    \bsCODE{xpos=(p+1)//2+i}
    \bsCODE{upref[q][p]+=t_i*ref2[q][\clip(xpos,0,\height(ref)-1)]}
  \bsEND
  \bsCODE{upref[q][p] += 128}
  \bsCODE{upref[q][p] \gg=8}
